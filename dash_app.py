...
app.layout = html.Div([
    ...
    html.Div(
        [
            ...
            dcc.Input(id = 'text-input', type = 'text', value = "AUDCAD")
        ],
    ...
    html.Div(id='output-instructions', children='Enter a currency code and press submit')
    ,
    html.Div([
    ...
    html.Div(dcc.Input(id = 'currency-pair-number', type = 'number')),
    # Submit button for the trade
    html.Button('Trade', id='trade-submit-button', n_clicks=0)
...
@app.callback(
    ...
    dash.dependencies.Output('output-instructions', 'children'),
...
def update_candlestick_graph(n_clicks, value): # n_clicks doesn't get used, we only include it for the dependency.
    ...
    file_to_read_from = open("currency_pair_history.csv", 'r')
    historydata = pd.read_csv("currency_pair_history.csv")
    ...
    fig.update_layout(title='CurrencyExchange')
...
@app.callback(
    ...
    [dash.dependencies.State('radios','value')
    ,
    dash.dependencies.State('currency-pair','value')
    ,
    dash.dependencies.State('currency-pair-number','value')
    ],
